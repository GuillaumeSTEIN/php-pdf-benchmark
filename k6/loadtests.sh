...
docker run --rm -i --network="host" grafana/k6 run -e URL=http://localhost:3003 - <k6/sequential.js
...
echo '' >> docker_stats.txt;
echo '    php-puppeteer' >> docker_stats.txt;
docker run --rm -i --network="host" grafana/k6 run -e URL=http://localhost:3003 - <k6/concurrent.js
